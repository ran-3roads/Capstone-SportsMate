...
export default function Manage() {
  ...
  if (mode == "PERMISSION") {
    managecontent = (
      <div>
        <div id="comment_count">
          승인대기중인 가입수
          {permissions.filter((p) => p.state == "WAITING").length}개
        </div>
                  ...
                  <CommonTableColumn>
                    ...
                    <button
                      ...
                      onClick={(event) => {
                        ...
                        axios
                          .post(
                            `/party/${id}/applyList/${event.target.id}/accept`
                          )
                          .then(function (response) {
                            //받는거
                            if (response.status == 200) {
                    ...
                    <button
                      ...
                      onClick={(event) => {
                        ...
                        axios
                          .post(
                            `/party/${id}/applyList/${event.target.id}/reject`
                          )
                          .then(function (response) {
                            //받는거
                            if (response.status == 200) {
    ...
    managecontent = (
      <div>
        <div id="comment_count">전체 파티원수 {members.length} 명</div>
                  ...
                  <CommonTableColumn>
                    ...
                    <button
                      ...
                      onClick={(event) => {
                        event.preventDefault();
                        axios
                          .delete(`/party/${id}/member/${event.target.id}`)
                          .then(function (response) {
                            //받는거
                            if (response.status == 200) {
                              alert("멤버를 추방하였습니다.");
